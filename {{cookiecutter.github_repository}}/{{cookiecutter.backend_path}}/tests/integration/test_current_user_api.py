...
def test_get_current_user_api(client):
    url = reverse("me")
    user = f.create_user(username="username", email="test@example.com")
...
def test_patch_current_user_api(client):
    url = reverse("me")
    user = f.create_user(username="panttojo", email="test@example.com", first_name="test", last_name="test")
    ...
    assert response.data["username"] == "modified_username"
